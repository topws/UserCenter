...
open class ElasticRefreshControl: UIView {
	...
	open override func layoutSubviews() {
		...
		spinner.center = CGPoint.init(x: self.bounds.width / 2.0, y: 0.75 * totalHeight)
	}
	...
	open override func draw(_ rect: CGRect) {
		...
		let upCenter = CGPoint.init(x: centerX, y: (0.75 - 0.5 * adjustedProgress) * totalHeight)
		let upRadius = radius - radius * 0.3 * adjustedProgress
		let downRadius: CGFloat = radius - radius * 0.75 * adjustedProgress
		let downCenter: CGPoint = CGPoint(x: centerX, y: totalHeight - downRadius - margin)
